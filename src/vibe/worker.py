...
def _execute_task(
    ...
    if (
        ...
        result = manager.run_plan(prompt, cwd=cwd, timeout=config.timeout)
    else:
        result = manager.run_task(prompt, cwd=cwd, timeout=config.timeout)
...
def _execute_with_approval(
    ...
    # Step 1: 生成计划
    plan_result = manager.generate_plan(prompt, cwd=cwd, timeout=timeout)
    ...
    exec_result = manager.execute_plan(plan_text, cwd=cwd, timeout=remaining_timeout)
